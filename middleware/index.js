...
middlewareObj.checkUserAccountOwnership = (req, res, next) => {
    if (req.isAuthenticated()) {
        User.findById(req.params.id, (err, user) => {
            if (err || !user) {
                ...
                if(user._id.equals(req.user.id)){
                    ...
                    req.flash("error", "You are not authorised to perform this function");
                    return res.redirect("/campgrounds");
                }
            }
        });
    } else {
        req.flash("error", "You are not authorised to make these changes");
        res.redirect("/login");
    }
}
